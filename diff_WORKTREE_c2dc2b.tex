...
%DIF DEL tmp2.tex   Mon Sep 14 02:25:03 2026
%DIF ADD tmp1.tex   Mon Sep 14 02:25:03 2026
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% %DIF > 
%           SUBMISSION RECORDS             %DIF > 
%  %DIF > 
% c2dc2b74036b7f6f303af9a920b30338ef928af5 %DIF > 
% 29622f7f6a58768cd541e613ea9a4c1b91ad89ec %DIF > 
% %DIF > 
% 1st submission (including response letter) %DIF > 
% - %DIF > 
%  %DIF > 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% %DIF > 
 %DIF > 
...
\DIFaddend \bstctlcite{IEEEexample:BSTcontrol}
...
.
These constraints often manifest as convex input constraints, which can be in the form of nonlinear constraints rather than the element-wise (symmetric or asymmetric) constraints that limit each input independently, }\ie \DIFadd{$\underline{u}_i\le u_i\le\overline{u}_i$, $\forall i\in\{1,\cdots,n\}$, where $\underline{u}_i,\overline{u}_i\in\R$ denote the lower and upper bounds of the $i$th input $u_i$, respectively.
For example, under road friction limits, the admissible global force and moment of vehicles can be characterized by a weighted norm bound, which yields a convex ellipsoidal constraint \mbox{%DIFAUXCMD
...
Conventionally, SHL and RBF NNs are often employed to approximate uncertain or unknown system dynamics or controllers due to their simplicity \mbox{%DIFAUXCMD
...
}\DIFdelend \DIFaddbegin \DIFadd{to approximate the dynamics in a structured and parameterized representation \mbox{%DIFAUXCMD
...
Most studies derived these laws using Lyapunov-based methods, ensuring the boundedness of the tracking error and weight estimation error, thus maintaining systemstability under uncertainty.
}%DIFDELCMD < 
...
The following section provides a detailed }\DIFdelend \DIFaddbegin \DIFadd{Because multiple convex input constraints and uncertain or unknown dynamics often coexist in real-world applications, incorporating multiple input constraint handling into NAC is essential.
...
\DIFdelbegin \DIFdel{A common challenge in NAC is maintaining the boundedness of the NN weights to ensure stability.
...
However, these constants were often selected as large as possible due to }\DIFdelend \DIFaddbegin \DIFadd{In Table~\ref{table:intro:comparison}, we summarize input constraint handling methods reported in }\DIFaddend the \DIFdelbegin \DIFdel{lack of theoretical guarantees regarding the global optimality of the weight values.
While this approach ensured that the NN remained stable, it did not necessarily result in optimal performance.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{In addition to projection operators, some studies utilized modification techniques like $\sigma$-modification \mbox{%DIFAUXCMD
...
}\DIFdelend \DIFaddbegin \DIFadd{literature, which can be broadly categorized into structural methods and optimization-based methods.
In subsequent subsections, these methods are reviewed, highlighting their advantages and limitations.
Additionally, the input constraints also referred to as input saturation to broadly refer to limitations imposed on the control input.
}\DIFaddend 
...
\DIFdelbegin \subsubsection{\DIFdel{Satisfying Input Constraints}}
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
\DIFdelend %DIF >  Since the NAC framework is based on Lyapunov theory, structural methods have been widely studied for input constraint handling.
...
In \mbox{%DIFAUXCMD
\cite{Ji:2025aa}}\hskip0pt%DIFAUXCMD
, Ji }\etal \DIFadd{categorized the literature on the structural methods into two strategies: anti-saturation design and direct design.
We review these strategies in the context of NAC.
}\DIFaddend 

\DIFdelbegin \DIFdel{The second major issue is satisfying input constraints, particularly in systems where actuators are subject to physical limitations. 
...
In such cases, if the system model is inaccurate or the dynamics targeted by the NN are already contributing to stability, the combined control effort may result in overly aggressive inputs, increasing the risk }\DIFdelend \DIFaddbegin \DIFadd{In anti-saturation design, a virtual auxiliary system is mainly constructed to compensate for the effects of input saturation \mbox{%DIFAUXCMD
...
For example, auxiliary systems have been incorporated into backstepping-based NAC \mbox{%DIFAUXCMD
\cite{Gao:2017aa} }\hskip0pt%DIFAUXCMD
and sliding mode control (SMC)-based NAC \mbox{%DIFAUXCMD
\cite{Ma:2025aa} }\hskip0pt%DIFAUXCMD
to mitigate the effects }\DIFaddend of input saturation\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
Particularly, the }\DIFdelend \DIFaddbegin \DIFadd{.
...
}\DIFaddend auxiliary systems have been \DIFdelbegin \DIFdel{widely introduced.
In \mbox{%DIFAUXCMD
\cite{Arefinia:2020aa,He:2016aa,Peng:2020aa}}\hskip0pt%DIFAUXCMD
, the auxiliary states were generated whenever input saturation was detected, and these states were used as feedback termsin the control lawto directly compensate for the effects }\DIFdelend \DIFaddbegin \DIFadd{proposed.
}

\DIFadd{Generally, when the control input reaches the saturation limit, the auxiliary system is excited, and the auxiliary state is fed back into the control law as additional compensation terms.
...
Therefore, the auxiliary state is often considered in the adaptation law \mbox{%DIFAUXCMD
...
%DIFDELCMD < %%%
\DIFdel{However, these approaches typically handle input bound constraints on a per-input basis, }%DIFDELCMD < \ie %%%
\DIFdel{applying bounds to each scalar control variable individually, and may not account for more complex, nonlinear constraints, like input norm constraints, which are commonly found in physical systems such as robotic actuators or motor systems}\DIFdelend \DIFaddbegin \DIFadd{controller design and stability analysis.
...
, the smooth approximation of the saturation function may introduce conservative behavior and performance degradation, especially when the control input is close to the saturation limit, compared to the auxiliary system-based anti-saturation design}\DIFaddend .
...
}\DIFdelend \DIFaddbegin \DIFadd{Another approach can be found in reinforcement learning (RL)-based NAC \mbox{%DIFAUXCMD
\cite{Zhang:2024ab,Qin:2025ab}}\hskip0pt%DIFAUXCMD
.
The input saturation is handled by introducing a nonquadratic input cost into the infinite-horizon optimal control problem, which is solved using adaptive dynamic programming (ADP).
Within the resulting ADP framework, the optimal cost function is approximated by a critic neural network (CNN), and the optimal control law is derived from the Hamilton-Jacobi-Isaacs (HJI) equation or the Hamilton-Jacobi-Bellman (HJB) equation.
Owing to the nonquadratic input cost, the resulting optimal control law is based on $\tanh(\cdot)$ and satisfies asymmetric or symmetric input bounds.
}\DIFaddend 
...
Constrained optimization provides a theoretical frameworkfor defining optimality and presents numerical methods for finding solutions that satisfy the constraints \mbox{%DIFAUXCMD
...
%DIF <  \ctxt{blue}
%DIF <  {
\DIFdelbeginFL \DIFdelFL{However, there are few works that have applied constrained optimization to NAC systems with real-time weight adaptation \mbox{%DIFAUXCMD
...
%DIFDELCMD <     \item %%%
\DIFdel{A constrained }\DIFdelend %DIF >  MPC 
%DIF >  Bemporad:1999aa,Berberich:2025aa,Draeger:1995aa,Chen:2024aa,Cetin:2019aa,Vahidi-Moghaddam:2025aa
\DIFaddbegin 

\subsubsection{\DIFadd{Optimization-Based Methods for Input Constraint Handling}}
...
These methods compute the control input by solving constrained optimization problems at each time step, as in model predictive control (MPC) \mbox{%DIFAUXCMD
...
\DIFadd{Even though they solve constrained optimization problems at each time step, the computational burden may not necessarily be significant for small-scale QPs or MPC problems with short horizons and simple constraints.
...
%DIF >  In such cases, real-time implementation may require iterative numerical solvers, suitable initialization, and careful control of the problem dimension and solver convergence.
...
Accordingly, }\DIFaddend neuro-adaptive \DIFdelbegin \DIFdel{control (CONAC) is proposed , where the control }\DIFdelend \DIFaddbegin \DIFadd{system identifiers have been introduced to construct the CBF-QP filters using approximated system dynamics \mbox{%DIFAUXCMD
\cite{Sweatland:2025aa}}\hskip0pt%DIFAUXCMD
.
}

\DIFadd{Although these approaches can provide stability or safety guarantees under their stated assumptions, the guarantees typically depend on feasibility, prediction, or safety-critical filters whose validity is affected by model accuracy, uncertainty descriptions, data quality, and offline training coverage.
}
...
\DIFadd{These observations motivate a unified NAC framework in which more complex input constraints are incorporated directly into the online adaptation law, as presented in the previous work \mbox{%DIFAUXCMD
\cite{Ryu:2025aa}}\hskip0pt%DIFAUXCMD
.
...
The constrained optimization formulation is not used to solve a control allocation problem at each time step; instead, it is used to derive continuous-time adaptation law for the adaptive variables.
...
}\DIFdelend \DIFaddbegin \DIFadd{framework combines the Lyapunov-based stability analysis of structural methods with the broader input-constraint handling capability of optimization-based methods, particularly for multiple convex input constraints.
}
...
\DIFadd{The main contributions of this study are:
  ...
  \DIFaddend \item 
  The adaptation \DIFdelbegin \DIFdel{laws for }\DIFdelend \DIFaddbegin \DIFadd{law for adaptive variables, including the }\DIFaddend DNN weights and \DIFdelbegin \DIFdel{Lagrange multipliers are systematically derived to solve the defined problem, using constrained optimization theory. These laws guarantee convergence to the first-order optimality conditions at steady state, specifically the Karush–Kuhn–Tucker }\DIFdelend \DIFaddbegin \DIFadd{the Lagrange multipliers, is systematically derived from the formulated constrained optimization problem, and the Karush-Kuhn-Tucker }\DIFaddend (KKT) \DIFdelbegin \DIFdel{conditions, as described in }\DIFdelend \DIFaddbegin \DIFadd{optimality conditions are satisfied at an equilibrium of the adaptation dynamics }\DIFaddend \cite[Chap. 12, Thm. 12.1]{Nocedal:2006aa}\DIFdelbegin \DIFdel{.
...
%DIFDELCMD < %%%
\DIFdelend This study extends \DIFdelbegin \DIFdel{our }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend preliminary work in \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Ryu:2025aa}}\hskip0pt%DIFAUXCMD
...
%DIF >  \subsection*{Notations}
\DIFaddbegin \noindent\textit{\DIFadd{Notation:}}
...
The operator $\myproj_{h(\mv{z})\le 0}(\mv{\nu})$ denotes the projection of the direction $\mv{\nu}\in\R^n$ according to a convex set $\mathcal{C}(\mv{z}):=\left\{ \mv{z}\in\R^n: h(\mv{z})\le 0 \right\}$, where $h:\R^n\to\R$ is a convex function, following the projection rule in \mbox{%DIFAUXCMD
\cite[Appendix E, Eq. (E.4)]{Krstic:1995aa}}\hskip0pt%DIFAUXCMD
.
}

\DIFaddend \subsection{Organization}
...
In Section \ref{sec:adap_laws}, }\DIFdelend \DIFaddbegin \DIFadd{, including }\DIFaddend the adaptation law \DIFdelbegin \DIFdel{is developed.
Section\ref{sec:stability} analyzes the stability of CONAC.
Section\ref{sec:sim} }\DIFdelend \DIFaddbegin \DIFadd{for adaptive variables.
Section~\ref{sec:stability:analysis} analyzes the closed-loop stability.
Section~\ref{sec:val} }\DIFaddend presents a real-time implementation of CONAC on a 2-DOF robotic manipulator\DIFaddbegin \DIFadd{, compares it with existing methods, and discusses its advantages and limitations}\DIFaddend .
Finally, Section\DIFaddbegin \DIFadd{~}\DIFaddend \ref{sec:conclusion} concludes the paper and discusses potential future \DIFdelbegin \DIFdel{work.
Candidates for convex input constraints are provided in Appendix \ref{sec:appen:cstr}. 
}\DIFdelend \DIFaddbegin \DIFadd{works.
}\DIFaddend 
...
\DIFdelbegin %DIFDELCMD < \begin{prop}
%DIFDELCMD <     %%%
\DIFdel{The disturbance $\rbd$ is bounded}\DIFdelend \DIFaddbegin \begin{assum}[{\cite[Assump. 1(b)]{Boulkroune:2010aa}}] \label{assum:B:bound:grad}
  \DIFadd{There exists an unknown positive function $\rho_{\beta}(\mv{x}):\R^n\to\R_{>0}$ such that $\tfrac{1}{2}\mv{v}^\top\left(\ddtt\mm{B}(\mv{x})^{-1}\right)\mv{v} \le \rho_{\beta}(\mv{x})\norm{\mv{v}}^2$, $\forall \mv{v}\in\R^n$, and $\forall \mv{x}\in\R^n$. 
}\end{assum}

\begin{assum} \label{assum:sat}
  \DIFadd{The convex admissible input set $\Omega_u$ and saturation function satisfy the following conditions:
  }\begin{enumerate}
    \item
    \DIFadd{The admissible input set $\Omega_u$ is defined by smooth convex constraint functions $c_j^u:\R^n\to\R,\ \forall j\in\mathcal I$, as
    }\begin{equation*}
      \Omega_u
      =
      \textstyle
    ...
    \item
    \DIFadd{The origin lies in the interior of $\Omega_u$}\DIFaddend , \ie
...
\DIFdelend \DIFaddbegin \begin{equation*}
      ...
      ,
    \end{equation*}
    \DIFadd{where $\underline c_j^u\in\R_{>0},\ \forall j\in\mathcal I,$ is known.
}\DIFaddend 
...
\DIFdel{$\norm{\qd(t)}\le \overline{q}_d\in\R_{>0}$, and available for designing a boundedcontrol input}\DIFdelend \DIFaddbegin \DIFadd{$\mv{x}_d:=\mv{x}_d(t)$ is sufficiently smooth, and $\mv{x}_d$ and $\ddtt\mv{x}_d$ are bounded}\DIFaddend .
...
\DIFaddbegin \DIFadd{The control objective is to design a NAC for the unknown system \eqref{eq:sys} such that $\mv{x}$ tracks the desired trajectory $\mv{x}_d$, the multiple convex input constraints defined by $\mysat(\cdot)$ are satisfied, and closed-loop stability is guaranteed.
}
...
Then, the controller is presented in Section~\ref{sec:main:result:sub:ctrl:design} along with the adaptation law in Section~\ref{sec:main:result:sub:adap:laws}}\DIFaddend .
...
\cite{Patil:2022aa}}\hskip0pt%DIFAUXCMD
, the DNN $\NN: \mathbb{R}^{l_0+1} \times \mathbb{R}^{\Xi} \to \mathbb{R}^{l_{k+1}}$ can be used to approximate any smooth unknown nonlinear function $\mv{g}:\R^{l_0+1}\to\R^{l_{k+1}}$.
...
%     h(\mv{x},\mv{x}_d)
  ...
  .
\end{align}
\DIFadd{Notably, in the ideal case, }\ie \DIFadd{$\mv{u} = \mv{u}_d$ and $\Delta\mv{u} = \mv{0}$, the tracking error $\mv{e}$ asymptotically converges to zero with decay rate $\underline{p}\overline{b}$, since 
$
  \tfrac{1}{\overline{b}}
  \ddtt
  \left(
...
\DIFaddbegin \DIFadd{Since the ideal control law \eqref{eq:ctrl:ideal} contains unknown functions $\rho_{\beta}(\mv{x})$, $\mv{f}$ and $\mm{B}$, we employ the DNN $\NN(\NNin;\wth)$ in \eqref{eq:dnn:def} to approximate the ideal control law as follows:
}\begin{equation*}
% \begin{aligned}
  \NN(\NNin;\idealwth)
  +
...
\DIFaddend The control objective \DIFdelbegin \DIFdel{can be represented as follows:
}%DIFDELCMD < \begin{equation}
...
%DIFDELCMD < \end{equation}%%%
\DIFdelend \DIFaddbegin \DIFadd{defined in Section~\ref{sec:problem:formulation} can be formulated as a constrained optimization problem, as follows:
...
The adaptation \DIFdelbegin \DIFdel{laws for }\DIFdelend \DIFaddbegin \DIFadd{law for the adaptive variables, }\DIFaddend $\estwth$\DIFdelbegin \DIFdel{and $[\lambda_j]_{j\in\mathcal I}$ are }\DIFdelend \DIFaddbegin \DIFadd{, $[\sigma_i]_{i\in\mathcal{K}}$, and $[\lambda_j]_{j\in\mathcal{I}}$, is }\DIFaddend derived to solve \DIFdelbegin \DIFdel{the dual problem of \eqref{eq:opt:problem1}}\DIFdelend \DIFaddbegin \DIFadd{\eqref{eq:opt:prob:saddle} by using a primal-dual gradient update law}\DIFaddend , \ie 
...
}\DIFdelend \DIFaddbegin \DIFadd{$\myproj_{(\cdot)\ge0}(\cdot)$ in \eqref{eq:adap:sig} and \eqref{eq:adap:lbd}.
}

\DIFaddend When a constraint \DIFdelbegin \DIFdel{$c_j$ becomes active}\DIFdelend \DIFaddbegin \DIFadd{becomes violated}\DIFaddend , \ie \DIFdelbegin \DIFdel{$c_j > 0$}\DIFdelend \DIFaddbegin \DIFadd{positive constraint violation $c^{\theta}_{i},c^{u}_{j}>0$}\DIFaddend , the corresponding Lagrange multiplier \DIFdelbegin \DIFdel{$\lambda_j$ increases , see }\DIFdelend \DIFaddbegin \DIFadd{increases according to \eqref{eq:adap:sig} and }\DIFaddend \eqref{eq:adap:lbd}, to address the violation \DIFdelbegin \DIFdel{, see \eqref{eq:adap:th}.
...
\DIFdelbegin \DIFdel{At steady state, where $\ddtt{\estwth}=0$ }\DIFdelend \DIFaddbegin \begin{remark}[Comparison with Existing Methods for Weight Boundedness] \label{rem:weight:bound}
  \DIFadd{Unlike projection-based methods that directly restrict the adaptive weights to a prescribed set, or $\sigma$-modification }\DIFaddend and \DIFdelbegin \DIFdel{$\ddtt\lambda_j=0$, the KKT conditions are satisfied, }%DIFDELCMD < \ie %%%
...
}\DIFdelend \DIFaddbegin \DIFadd{$\epsilon$-modification that continuously bias them toward the origin \mbox{%DIFAUXCMD
...
\DIFdelbegin \DIFdel{The recommended method to calculate the exact value of $\pptfrac{\fe}{\rbu}$ is to use the }\DIFdelend %DIF >  Exact computation methods, such as the forward sensitivity method \cite{Khalil:2002aa,Ryu:2025ab}, can be used in principle, but they are often computationally demanding for real-time implementation with numerous DNN weights.
\DIFaddbegin \DIFadd{Exact computation methods, such as the }\DIFaddend forward sensitivity method \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
, can be used in principle, but they require exact knowledge of the system dynamics which is not available in this study.
On the other hand, several approximation methods have been proposed \mbox{%DIFAUXCMD
\cite{Douratsos:2007aa,Saerens:1991aa}}\hskip0pt%DIFAUXCMD
.
...
%DIFDELCMD <     %%%
\DIFdel{The constraint function $c_j(\estwth)$ is convex in the (input-) $\tau$-space and satisfies $c_j(0) \le 0$ }\DIFdelend \DIFaddbegin \DIFadd{Additionally, the adaptation law \eqref{eq:adap} provides an optimality interpretation at equilibrium.
...
Thus, the equilibria characterize candidates for locally optimal solutions of the formulated constrained optimization problem.
}\DIFaddend 
...
\DIFdel{$\fe^{(k)} \leftarrow \ddtt{\mv e^{(k)}}+\mm\Lambda \mv e^{(k)}$ based on \eqref{eq:err:filtered}
      ...
      \STATE \textit{// Neuro-adaptive control law (Section\DIFdelbegin \DIFdel{\ref{sec:ctrl design}}\DIFdelend \DIFaddbegin \DIFadd{~\ref{sec:main:result:sub:ctrl:design}}\DIFaddend )}
        ...
        \STATE \DIFdelbegin \DIFdel{$\rbu^{(k)}\leftarrow\NN({\q}_n^{(k)};\estwth^{(k)}) $ based on \eqref{eq:control:est}
                }\DIFdelend \DIFaddbegin \DIFadd{$\mv{u}^{(r)}\leftarrow\mm{P}\NN(\NNin^{(r)};\estwth^{(r)}) $ based on \eqref{eq:ctrl:NN:actual}
      ...
      \STATE \textit{// \DIFdelbegin \DIFdel{Train DNN }\DIFdelend \DIFaddbegin \DIFadd{Update Adaptive Variables }\DIFaddend (Section\DIFdelbegin \DIFdel{\ref{sec:adap_laws}}\DIFdelend \DIFaddbegin \DIFadd{~\ref{sec:main:result:sub:adap:laws}}\DIFaddend )}

      \STATE Compute \DIFdelbegin \DIFdel{$\pptfrac{\NN}{\estwth^{(k)}}$ }\DIFdelend \DIFaddbegin \DIFadd{$\pptfrac{\estNN^{(r)}}{\estwth^{(r)}}$ }\DIFaddend as defined in \eqref{eq:gradient:phi}
...
}\DIFdelend \DIFaddbegin \DIFadd{Compute ${c^{\theta}_i}^{(r)},{c^{u}_j}^{(r)}$ and $\pptfrac{{c^{u}_{j}}^{(r)}}{\estwth^{(r)}}$ for all $i\in\mathcal{K}, j\in\mathcal{I}$
      }\DIFaddend 
...
\begin{theorem} \DIFdelbegin \DIFdel{For the dynamical system described in \eqref{eq:sys1}, the NAC in \eqref{eq:control:est} with the weight adaptation laws in \eqref{eq:adaptation law} ensure the boundedness of the filtered error $\fe$ and }\DIFdelend \DIFaddbegin \label{thm:stability}
  \DIFadd{Consider the closed-loop system consisting of \eqref{eq:sys}, the control law \eqref{eq:ctrl:NN:actual}, and the adaptation law \eqref{eq:adap}.
...
\DIFdel{if $\fe$ leaves $\Theta_{\fe}^{1}$, $\ddtt V_1$ is negative, can be obtained using Lemma \ref{lem:stable:set} }\DIFdelend \DIFaddbegin \DIFadd{$V_{k}$ can be analyzed as follows:
  ...
  .
\end{align}
\DIFadd{The projection operators in \eqref{eq:adap:sig} and \eqref{eq:adap:lbd} are omitted according to Lemma~\ref{lem:proj:ineq}.
}

\DIFadd{For the boundedness of $\idealwth_k$ and $\mv{\epsilon}$, we assume that $\NNin$ remains in the approximation region $\Omega_{\NNin}$, which will be ensured later in the proof.
...
\DIFadd{Using Lemma~\ref{lem:delta:u:bound}, we can further analyze $\ddtt V_{k}$ }\DIFaddend as follows:
\DIFdelbegin %DIFDELCMD < \begin{equation}
...
%DIFDELCMD < \end{equation}%%%
\DIFdelend \DIFaddbegin \begin{align} \label{eq:Vk:dot:2} 
  \ddtt V_{k}
  \le&
    -\underline{p}
    \norm{\mv{e}}^2
...
%DIFDELCMD < \end{equation}%%%
\DIFdelend \DIFaddbegin \DIFadd{For further analysis, we use Young's inequality which states 
%DIF >  \begin{equation}
$
  ab
  \le
...
By applying Young's inequality to the third term in the right-hand side of \eqref{eq:Vk:dot:2}, we have
  ...
  ,
\end{align}\DIFaddend 
\DIFdelbegin \DIFdel{According to \eqref{eq:lya2:dot} and Lemma \ref{lem:stable:set}, the invariant set of $\estwth_k$ is defined as
}%DIFDELCMD < \begin{equation}
...
%DIFDELCMD < \end{equation}%%%
\DIFdelend \DIFaddbegin \DIFadd{where $\eta_1,\eta_2\in\R_{>0}$ and $\sigma_k^o\in\R_{>0}$ are arbitrarily chosen such that $\underline{p}-\eta_1-\eta_2>0$ for given $\underline{p}$, and $\sigma_k^o - \frac{1}{4\eta_1} \overline{p}^2 (l_k+1) > 0$, respectively.
}

\DIFadd{This result implies that $\ddtt V_{k}$ is negative outside the compact set defined by 
}\begin{equation} 
  \Omega_k
  \!
  =
...
Because $\ddtt V_k<0$ for all $\mv{z}_k\notin\Omega_k$, $\mv{z}_k$ initialized in $\mathcal{S}_k(\overline{V}_k)$ will remain in $\mathcal{S}_k(\overline{V}_k)$ for all time and eventually enter $\Omega_k$.
...
\DIFdel{$c_j,\ \forall j\in\mathcal{I}\setminus \{\theta_i\}_{i\in\{0,\cdots,k-1\}}$, can be verified through }\DIFdelend \DIFaddbegin \DIFadd{In addition, }\DIFaddend the boundedness of \DIFdelbegin \DIFdel{$\estwth_k$.
For the output layer's weight norm constraint $c_{\theta_k}$, if the corresponding Lagrange multiplier $\lambda_{\theta_k}$ increases sufficiently large due to the constraint violation, $\estwth_k$ approaches toward the origin as
the invariant set $\Theta_{\theta_k}^{1}$ shrinks to a point.
The remaining input constraints can be validated implicitly through the term ${\gamma}_2$ in \eqref{eq:lya2:dot}.
Similarly to $c_{\theta_k}$, when an input constraint $c_j,\ \forall j\in\mathcal{I}\setminus \{\theta_i\}_{i\in\{0,\cdots,k\}}$, is violated, the corresponding Lagrange multiplier $\lambda_j$ increases sufficiently to a large value, leading to an increase in ${\gamma}_2$.
...
}\DIFdelend \DIFaddbegin \DIFadd{$\mv{z}_k$ implies that $\mv{x}$ is bounded, since $\mv{e}=\mv{x}-\mv{x}_d$ and $\mv{x}_d$ is bounded by Assumption~\ref{assum:xd}.
Therefore, since $\mv{z}_k$ remains bounded in $\mathcal S_k(\overline{V}_k)$, all components of $\NNin$ remain bounded, implying there exists a compact approximation region $\Omega_{\NNin}$.
This justifies the boundedness of $\idealwth_k$ and $\mv{\epsilon}$ used in this analysis.
}\DIFaddend 

\DIFdelbegin \DIFdel{Therefore, }\DIFdelend \DIFaddbegin \hfill

\textit{\DIFadd{Step~2: Boundedness of the inner-layer adaptive variables.}}
...
Specifically, for an arbitrary layer index $i\in\{0,\cdots,k-1\}$, we assume that the subsequent adaptive variables, $\estwth_{i+1},\sigma_{i+1},\cdots,\estwth_k,\sigma_k,$ have already been shown to be bounded, and then prove }\DIFaddend the boundedness of \DIFdelbegin \DIFdel{$\fe$ }\DIFdelend \DIFaddbegin \DIFadd{$\estwth_i$ }\DIFaddend and \DIFdelbegin \DIFdel{$\estwth_k$ is guaranteed by the invariant sets $\Theta_{\fe}^{1}$ }\DIFdelend \DIFaddbegin \DIFadd{$\sigma_i$.
}

\DIFadd{Consider the Lyapunov function of the $i$th layer defined as
...
$
  ...
  ,
$
}\DIFaddend and \DIFdelbegin \DIFdel{$\Theta_{\theta_k}^{1}$, and the satisfaction of $c_j,\ \forall j\in\mathcal{I}\setminus \{\theta_i\}_{i\in\{0,\cdots,k-1\}}$, is ensured.
}\DIFdelend \DIFaddbegin \DIFadd{$\sigma_i^o\in\R_{>0}$ will be determined in the subsequent analysis.
...
%DIFDELCMD < %%%
\DIFdel{Since the input constraints are inactive, }\DIFdelend \DIFaddbegin \DIFadd{To establish }\DIFaddend the \DIFdelbegin \DIFdel{saturation function $\mysat(\cdot)$ in \eqref{eq:lya1:dot2} and the input constraint functions $c_j,\ \forall j\in\mathcal I \setminus \{\theta_i\}_{i\in\{0,\cdots,k\}}$}\DIFdelend \DIFaddbegin \DIFadd{boundedness of $\mv{\gamma}_i$, we note that: 
(i) $\pptfrac{\estNN}{\estwth_i}$ is bounded because the subsequent weights $\estwth_{i+1},\ldots,\estwth_k$ are bounded under the recursive assumption; 
(ii) $\mv e$, $\estwth_k$, and $[\lambda_j]_{j\in\mathcal I}$ are bounded by Step~1; and 
(iii) $\pptfrac{c_j^u}{\mv u}$ is bounded over the compact domain containing $\mv u$, owing to the smoothness of $c_j^u(\cdot)$ in Assumption~\ref{assum:sat} and the boundedness of $\mv u$ established in \eqref{eq:NN:bound}. 
Therefore, there exists an unknown constant $\overline{\gamma}_i\in\R_{>0}$ such that $\norm{\mv{\gamma}_i}\leq\overline{\gamma}_i$.
}

\DIFadd{Then, by applying Young's inequality to the coupled term, }\ie \DIFadd{$\norm{\estwth_i}\norm{\mv{\gamma}_i}\le\norm{\estwth_i}^2+\tfrac{1}{4}\overline{\gamma}_i^2$}\DIFaddend ,  in \DIFdelbegin \DIFdel{\eqref{eq:lya2:dot} can be neglected. 
...
%DIFDELCMD < \end{equation}%%%
\DIFdelend \DIFaddbegin \DIFadd{\eqref{eq:Vi:dot:1}, we have
  ...
  ,
\end{align}\DIFaddend 
\DIFdelbegin \DIFdel{Based on the same reasoning as in Case }\DIFdelend \DIFaddbegin \DIFadd{where $\delta_i:=\frac{1}{4}\overline{\gamma}_i^2 + \sigma_i^o \overline{\theta}_i^2$ is a positive constant, and $\sigma_i^o\in\R_{>0}$ is arbitrarily chosen such that $\sigma_i^o - 1 > 0$.
As in Step~}\DIFaddend 1, \DIFdelbegin \DIFdel{the boundedness of $\fe$ is ensured by the invariant set :
}%DIFDELCMD < \begin{equation}
...
%DIFDELCMD <     .
%DIFDELCMD < \end{equation}%%%
\DIFdelend \DIFaddbegin \DIFadd{$\ddtt V_i$ is negative outside the compact set defined by 
}\begin{equation}
  \Omega_i
  =
  \left\{
  ...
  , 
\end{equation}\DIFaddend 
\DIFdelbegin \DIFdel{The boundedness of $\estwth_k$ can also be ensured by the invariantset $\Theta_{\theta_k}^{2}=\{ \estwth_k \in \R^{\Xi_{k}} \mid \norm{\estwth_k} \le \overline\theta_k \}$, as $\lambda_{\theta_k}$ remains positive unless the weight norm constraint $c_{\theta_k}$ is satisfied.
}%DIFDELCMD < 
...
%DIFDELCMD < \end{equation}
%DIFDELCMD < %%%
\DIFdel{By Lemma \ref{lem:cstr:grad:bound}, }\DIFdelend \DIFaddbegin \DIFadd{which implies the boundedness of }\DIFaddend $\estwth_i$ \DIFdelbegin \DIFdel{is bounded, provided that $\estwth_i,\ \forall i\in\{k,\cdots,i+1\}$, are bounded. 
...
Moreover the satisfaction of the weight norm constraints for the inner layers can be verified, as the Lagrange multipliers $\lambda_{\theta_i}$, $\forall i\in\{k-1,\cdots,0\}$, increase sufficiently large to stabilize the $\estwth_i$ dynamics in \eqref{eq:estwth:dyn}}\DIFdelend \DIFaddbegin \DIFadd{and $\sigma_i$.
...
Since $\ddtt V_i<0$ for all $\mv z_i\notin\Omega_i$}\DIFaddend , \DIFdelbegin \DIFdel{leading }\DIFdelend \DIFaddbegin \DIFadd{$\mathcal S_i(\overline V_i)$ is positively invariant. 
Together with the previously established invariance of $\mathcal S_{i+1}(\overline V_{i+1}),\ldots,\mathcal S_k(\overline V_k)$, this implies that any trajectory initialized in $\prod_{\ell=i}^k \mathcal{S}_\ell(\overline V_\ell) = \mathcal S_k(\overline V_k)\times\cdots\times\mathcal S_i(\overline V_i)$ remains in this set for all time.
}

\DIFadd{By recursively applying the above analysis from $i=k-1$ }\DIFaddend to \DIFdelbegin \DIFdel{its convergence toward the origin}\DIFdelend \DIFaddbegin \DIFadd{$i=0$, the boundedness of the control system, including $\mv{e}$, $\estwth$, and $\sigma_i,\ \forall i \in \mathcal{K},$ and $\lambda_j,\ \forall j \in \mathcal{I},$ is established, which concludes the proof}\DIFaddend .
...
\DIFdelbegin \DIFdel{In conclusion, the filtered tracking error $\fe$ and the weight estimate $\estwth$ are bounded, }\DIFdelend %DIF >  ——————————————————————————————————————————————
%DIF >  REMARKS
%DIF >  ——————————————————————————————————————————————
\DIFaddbegin 
...
\begin{remark}[Initialization of CONAC] \label{rem:init}
  ...
  In practice, we recommend initializing; the system state $\mv{x}(0)$ close to the initial reference state $\mv{x}_d(0)$ to reduce the initial tracking error $\mv{e}(0)$; DNN weights with small random values; and zero for the Lagrange multipliers, $[\sigma_i(0)]_{i\in\mathcal{K}}=\mv{0}$ }\DIFaddend and \DIFdelbegin \DIFdel{all imposed constraints $c_j,\ \forall j\in\mathcal I$, are satisfied.
  }\DIFdelend \DIFaddbegin \DIFadd{$[\lambda_j(0)]_{j\in\mathcal{I}}=\mv{0},$ without constraint violations, to help ensure that the system starts within
  $
    \prod_{\ell=0}^k \mathcal{S}_\ell(\overline V_\ell)
    .
  $
...
\DIFdelend \DIFaddbegin \begin{remark}[Initial Desired Trajectory Design] \label{rem:ref}  
  ...
  Hence, it is preferable to design the desired trajectory such that the system starts in a sufficiently stable operating region (}\eg \DIFadd{near the stable equilibrium point), and use a conservative desired trajectory during an initial warm-up phase, as introduced in \mbox{%DIFAUXCMD
\cite{Ryu:2025aa}}\hskip0pt%DIFAUXCMD
.
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{remark}[Tuning of CONAC Parameters] \label{rem:tuning}
...
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{Therefore, $\alpha$, $\beta_i^\theta$, and $\beta_j^u$ should be selected to balance learning speed, prompt constraint enforcement, and smooth closed-loop behavior.
...
%DIF <  \includegraphics[width=\figSizeOneCol\linewidth]
        %DIFDELCMD < {
%DIFDELCMD <             %%%
\DIFdelFL{src/figures/exp\_set.drawio.png
        }%DIFDELCMD < }%%%
%DIF < 
    %DIFDELCMD < \caption{%
{%DIFAUXCMD
...
\DIFadd{The input saturation $\mysat(\mv{\tau})$ was defined as follows:
%DIF >   as a combination of the torque saturation of the second joint and the norm saturation of the control input, 
...
}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:exp:robot:sat}.
The saturation function can be decomposed into three convex input constraints, which can be physically interpreted as the upper and lower torque limit of the second joint and the power source limit, respectively, and are denoted by
  ...
  .
\end{equation}
\DIFadd{Accordingly, the input constraint index set is defined as $\mathcal{I} = \{\overline{\tau}_2, \underline{\tau}_2, \overline{\tau}\}$.
}\DIFaddend 
...
%DIF <  ------------------------------------
%DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\DIFaddbegin \DIFadd{The satisfaction of the LICQ of the considered constraints in \eqref{eq:val:cstr:input} is analyzed.
First, the gradient of the upper and lower torque limit constraints $c^u_{\overline{\tau}_2}$ and $c^u_{\underline{\tau}_2}$ are derived as follows:
}\begin{equation}
  \ppfrac{c^u_{\overline{\tau}_2}}{\estwth} 
  =
  -
  \ppfrac{c^u_{\underline{\tau}_2}}{\estwth} 
  =
  \ppfrac{(\mm{P}\estNN)}{\estwth}^\top
  \begin{pmatrix}
    0\\1
  \end{pmatrix}
  =
  \ppfrac{\estNN}{\estwth}^\top
  \mm{P}^\top
  \begin{pmatrix}
    0\\1
  \end{pmatrix}
  .
\end{equation}
\DIFadd{Next, the gradient of the power source limit constraint $c^u_{\overline{\tau}}$ is derived as follows:
}\begin{equation}
  \ppfrac{c^u_{\overline{\tau}}}{\estwth}
  =
  \ppfrac{\mv{\tau}}{\estwth}^\top
  \mv{\tau}
  =
  \ppfrac{(\mm{P}\estNN)}{\estwth}^\top
  \mm{P}\estNN
  ...
  .
\end{equation}
\DIFadd{Since the upper and lower torque limit constraints are not active simultaneously, the gradients of the active constraints are linearly independent, satisfying the LICQ condition.
%DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
}
...
\DIFdelendFL \DIFaddbeginFL \multirow{3}{*}{
      ...
      \DIFdelbeginFL \DIFdelFL{\eqref{eq:cstr:input:bound} $c_{\overline\tau_2}$ and $c_{\underline\tau_2}$ }\DIFdelendFL \DIFaddbeginFL \multirow{3}{*}{
        \shortstack[l]{
    ...
    \DIFaddendFL \\
    & \DIFdelbeginFL \DIFdelFL{large update rate $\beta_j$. }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\beta^{u}_{\overline{\tau}_2}=\beta^{u}_{\underline{\tau}_2}=1000$, $\beta^{u}_{\overline{\tau}}=100,$ }\DIFaddendFL & 
...
\DIFdelendFL \DIFaddbeginFL \multirow{3}{*}{
    ...
    }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{CONAC with }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Proposed CONAC (smaller $\beta_j$) }\DIFaddendFL &  
    \DIFdelbeginFL \DIFdelFL{\eqref{eq:cstr:input:bound} $c_{\overline\tau_2}$ and $c_{\underline\tau_2}$ }\DIFdelendFL \\
    & \DIFdelbeginFL \DIFdelFL{small update rate $\beta_j$. }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\beta^{u}_{\overline{\tau}_2}=\beta^{u}_{\underline{\tau}_2}=50$, $\beta^{u}_{\overline{\tau}}=10,$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{with $\overline{\tau}_2=-\underline{\tau}_2=3.5$, and }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{same as (C$_1$) 
    }\DIFaddendFL \\ 
    & \DIFdelbeginFL \DIFdelFL{($\beta_{\overline{\tau}_2}=\beta_{\underline{\tau}_2}=1$ and $\beta_{\rbu}=0.1$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Others: same as (C$_1$}\DIFaddendFL ) & 
    \DIFdelbeginFL \DIFdelFL{\eqref{eq:cstr:input:norm} $c_{\rbu}$ with $\overline{\tau}=11$. }\DIFdelendFL \\
\hline
...
\cite{Rezaei:2025aa,Liu:2025aa,Yu:2022aa} 
    ...
    %DIF >  Others except for input constraints: same as (C$_1$)
    }\\
  \DIFaddendFL \hline
...
\cite{Patil:2022aa} }\hskip0pt%DIFAUXCMD
    ...
    \DIFdelbeginFL \DIFdelFL{$\overline{\tau}_1=-\underline{\tau}_1=10.428$ }\DIFdelendFL \DIFaddbeginFL \multirow{2}{*}{not considered}
    \DIFaddendFL \\
...
%DIFDELCMD < %%%
\DIFdel{To handle }\DIFdelend \DIFaddbegin \DIFadd{To handle all imposed }\DIFaddend input saturation, the proposed CONAC was implemented with the \DIFdelbegin \DIFdel{input bound constraints $c_{\overline\tau_2}$ and $c_{\underline\tau_2}$ \eqref{eq:cstr:input:bound} }\DIFdelend \DIFaddbegin \DIFadd{torque limit constraints $c^u_{\overline\tau_2}$ and $c^u_{\underline\tau_2}$ }\DIFaddend for the second link, and the \DIFdelbegin \DIFdel{input norm constraint $c_{\rbu}$ \eqref{eq:cstr:input:norm}, as follows:
}%DIFDELCMD < \begin{equation}
...
Therefore, controller (C$_1$)utilizes the full capability }\DIFdelend \DIFaddbegin \DIFadd{power source limit constraint $c^u_{\overline{\tau}}$ (defined in \eqref{eq:val:cstr:input}), allowing the controller to fully utilize the authority }\DIFaddend of the actuator\DIFdelbegin \DIFdel{, see \eqref{eq:sat:tau2} and \eqref{eq:sat:norm}.
...
As described in \DIFdelbegin \DIFdel{\eqref{eq:control:est}, the control }\DIFdelend \DIFaddbegin \DIFadd{\eqref{eq:ctrl:NN:actual}, the }\DIFaddend input (torque) corresponds to the output of the DNN, and the adaptation law is defined in \DIFdelbegin \DIFdel{\eqref{eq:adaptation law}.
}\DIFdelend \DIFaddbegin \DIFadd{\eqref{eq:adap}.
...
As a baseline for comparison \DIFaddbegin \DIFadd{in handling input constraints in NAC}\DIFaddend , the auxiliary \DIFdelbegin \DIFdel{system-based approach from \mbox{%DIFAUXCMD
\cite{Esfandiari:2014aa, Karason:1994aa, Esfandiari:2015aa} }\hskip0pt%DIFAUXCMD
was implemented.
...
It is notable that $\Delta_{\zeta}\mv{\tau}$ is defined as the element-wise input overflow, which is different from the actual input saturation $\Delta\mv{u}$ in \eqref{eq:Vc:dot:1}.
...
%DIF >  All other properties and parameters were kept the same as in (C$_1$)--(C$_3$), as summarized in Table~\ref{table:controller}.
}\DIFaddend 
...
The DNN weights were initialized with small random values according to $\estwth(0)\sim\mathcal{U}\bigl([-0.5,0.5]^{\Xi}\bigr)$, where $\mathcal{U}(\cdot)$ denotes a uniform random distribution, invoking Remark~\ref{rem:init}}\DIFaddend .
...
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \subfloat[Norm of the commanded control input $\mv{\tau}$.]{
    \includegraphics[width=.49\linewidth]
    {
      \resultFig/Fig7.eps
    }%
...
}\DIFdelend \DIFaddbegin \DIFadd{by around 25\% and 70\% for the first and second joint angles, respectively, demonstrating the adaptation capability of the DNN.
}\DIFaddend 
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \subfloat[Lagrange multiplier associated with the power source limit constraint for (C$_1$) and (C$_2$) during the time interval {$[\zoomStartTime, \qty{\zoomEndTime}{\second}]$}.]{
    \includegraphics[width=0.97\linewidth]
    {
      \resultFig/Fig9.eps
    }
...
\DIFdel{To further illustrate this effect, }\DIFdelend \DIFaddbegin \DIFadd{focusing on the multiplier associated with the power source limit constraint $\lambda_{\overline{\tau}}$.
...
~\ref{fig:ctrl:result:scope:beta}, where }\DIFdelend \DIFaddbegin \DIFadd{was observed to be non-zero, indicating that the power source limit constraint was active and enforced during this time interval.
...
Since the weights were adapted to reduce both the filtered error $\fe$ and the auxiliary state $\mv{\zeta}$, the control input saturation was subsequently reduced, as evident }\DIFdelend \DIFaddbegin \DIFadd{input constraint but induces greater oscillations in the Lagrange multiplier, whereas a smaller update rate yields smoother but less responsive constraint enforcement.
This result confirms the trade-off discussed in Remark~\ref{rem:tuning}.
}

\DIFadd{Additionally, the nearly constant value of $\lambda_{\overline{\tau}}$ }\DIFaddend in Fig.~\DIFdelbegin \DIFdel{\ref{fig:ctrl:result:scope:control}.
Once the saturation was resolved at around $31.2$ s (see }\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:ctrl:result:detail:multipliers} and the small oscillations of $c_{\overline{\tau}}^u$ in }\DIFaddend Fig.~\DIFdelbegin \DIFdel{\ref{fig:ctrl:result:tau:1}) , the auxiliary state $\zeta_1$ converged to zero due to the stable state matrix $\mm{A}_\zeta$ in the auxiliary system dynamics , given by $\ddtt\mv{\zeta} = \mm{A}_\zeta \mv{\zeta} + \mm{B}_\zeta \Delta\rbu$ with $\Delta\rbu=\mv{0}_{2\times1}$, as shown again in Fig.~\ref{fig:ctrl:result:scope:aux}.
}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:ctrl:result:detail:u:top:view} of (C$_2$) suggest that the adaptation dynamics remain near an equilibrium characterized by a nonzero Lagrange multiplier.
...
Although the adaptation dynamics do not converge exactly to a stationary KKT equilibrium because of the practical limitations in the real-time implementation, the observed behavior can be interpreted as a near-equilibrium behavior.
}\DIFaddend 
...
\DIFdelbegin \DIFdel{Finally, the handling of the weight norm constraints $c_{\theta_i},\ \forall i\in\{0,1,2\}$, is investigated. 
Among all controllers, the weight norm constraint was only activated for the output layer weights $\estwth_2$ in (C$_3$), as }\DIFdelend \DIFaddbegin \subsubsection{\DIFadd{Feasibility of CONAC in Real-Time Implementation}}
...
\DIFadd{The feasibility of the real-time implementation is analyzed in terms of the computation time.
As }\DIFaddend shown in Fig.~\DIFdelbegin \DIFdel{\ref{fig:ctrl:result:weight}. 
...
This was }\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:ctrl:result:cmp:time}, the computation time of all controllers is consistently below the sampling time of $\qty{500}{\micro\second}$, confirming the feasibility of real-time implementation at a sampling frequency of $\qty{500}{\hertz}$ ($\qty{2}{\milli\second}$ maximum sampling time).
This is }\DIFaddend attributed to the fact that the \DIFdelbegin \DIFdel{backpropagated error was scaled by the gradients of the activation function, which are bounded in $(0,1]$, }%DIFDELCMD < \ie %%%
\DIFdel{$\sigma(\cdot)=\tanh(\cdot)$}\DIFdelend \DIFaddbegin \DIFadd{proposed CONAC does not require solving an optimization problem at each time step, but rather continuously adapts the adaptive variables to solve the optimization problem in an online manner}\DIFaddend .

\DIFdelbegin \DIFdel{Additional details on the constraint handling process are provided in Fig.~\ref{fig:ctrl:result:scope:beta}. 
Similar to the case of the input constraints , the Lagrange multipliers $\lambda_{\theta_2}$ increased at around $30.4$ s, as shown in Fig.
~\ref{fig:ctrl:result:scope:beta}, when $\estwth_2$ exceeded the prescribed norm constraint $\overline\theta_2$ (see Fig.~\ref{fig:ctrl:result:weight}), thereby adjusting the adaptation direction of $\estwth_2$ to mitigate the }\DIFdelend \DIFaddbegin \subsection{\DIFadd{Discussion on the Advantages and Limitations of CONAC}}

\DIFadd{In the validations, we demonstrated that CONAC can handle multiple convex input constraints simultaneously, while the other NAC methods failed to do so.
However, it is important to note that the proposed CONAC is only able to handle the constraints in soft manner, meaning that the constraints may be violated during the adaptation process.
One may impose a conservative constraint considering the potential }\DIFaddend violation of the \DIFdelbegin \DIFdel{constraint $c_{\theta_2}$.
}%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdel{The effectiveness }\DIFdelend \DIFaddbegin \DIFadd{constraints, but this may lead to a suboptimal solution.
In safety perspective, some constraints must be satisfied without violation in operation time.
Consequently, extension }\DIFaddend of the proposed CONAC \DIFdelbegin \DIFdel{in real-time applications was evaluated by measuring the computational time using CPU clock ticks on the OpenCR1.0 board. 
As shown in Fig.
~\ref{fig:ctrl:real:result:cmp:time}, the computational times of all controllers remained below $4$ ms, demonstrating that the proposed CONAC is suitable for real-time implementation at a sampling rate of $250$ Hz.
}\DIFdelend \DIFaddbegin \DIFadd{to incorporate hard constraints remains an important direction for future work.
}\DIFaddend 
...
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Video Demonstration}}
\DIFaddend 

%DIF <   SECTION CONCLUSION ======================================
\DIFaddbegin \DIFadd{
%   For reproducibility, the source code and the experimental data are available in \mbox{%DIFAUXCMD
...
The \DIFaddbegin \DIFadd{adaptation law was derived from the constrained optimization problem, which was formulated to minimize the tracking error while satisfying the multiple convex input constraints.
...
By deriving adaptation law from the constrained optimization problem}\DIFaddend , CONAC ensured weight convergence in accordance with the \DIFdelbegin \DIFdel{Karush-Kuhn-Tucker (KKT ) }\DIFdelend \DIFaddbegin \DIFadd{KKT optimality }\DIFaddend conditions, thereby ensuring both stability and optimality.
...
Future work may extend this approach to \DIFdelbegin \DIFdel{include constraintson both control inputs and system states, further enhancing the flexibility and robustness of neuro-adaptive control systems within constrained optimization frameworks.
}\DIFdelend \DIFaddbegin \DIFadd{handle hard constraints, such as state constraints, in addition to input constraints. 
Moreover, by incorporating reinforcement learning into the optimization problem formulation and deriving adaptation law that explicitly accounts for disturbances, the proposed CONAC framework could be extended to infinite-horizon optimal control problems while further enhancing its robustness.
}\DIFaddend 
...
%DIFDELCMD < %%%
\DIFdel{This section introduces potential input constraintsthat can be incorporated into the proposed CONAC framework.
}%DIFDELCMD < 
...
\DIFaddbegin \bibliography{localRefs, fromLR, review_from_SMC/reviewRefs}
\DIFaddend 
