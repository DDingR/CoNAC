...
\begin{revresponse}[]
  ...
  We believe that these revisions have improved the technical rigor, significance, and presentation of the manuscript.
  ...
  \begin{enumerate}
    ...
    \item \textbf{Title and contribution clarification:} 
      ...
      % For example, the paper now emphasizes the capability to handle multiple convex input constraints instead of weight and input constraints, which is the main novelty of the proposed method.
      For example, the paper now emphasizes the capability to handle multiple convex input constraints, rather than the simultaneous handling of weight and input constraints, as the main novelty of the proposed method.
      Moreover, target system is generalized to unknown MIMO systems from unknown Euler-Lagrange systems.
...
\begin{revresponse}
  ...
  However, we recognized that, over the previously stated contribution, the significance of the proposed method is to incorporate multiple convex input constraints into the NAC framework, which typically considers only the element-wise input constraints, as compared in Fig.~1 of the revised manuscript, as shown below:
  \begin{center}
    \centering
    \includegraphics[width=0.65\linewidth]{figs/r1c1_1.png}
  \end{center}

  Moreover, to clarify the novelty of the proposed method, we have categorized the existing input-constraint handling approaches into two methods: Structural Methods and Optimization-Based Methods in introduction section of the revised manuscript, as shown in Table~I.
  \begin{center}
    \centering
  ...
  The justification of the novelty over our prior works \cite{Ryu:2025aa}, \cite{Ryu:2025ab} is also clarified in lines 212--222 of the revised manuscript, as follows:
  \begin{itemize}
    \item 
...
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
\begin{revcomment}[1][1]{}
	The adaptation law (16a) ((17a) in the present manuscript) relies on Lagrange multipliers generated from constraints, yet the convergence analysis does not establish whether the multipliers remain bounded or converge to optimal values. Without such guarantees, the claim of "bounded DNN weights" is incomplete and potentially misleading.
\end{revcomment}

\begin{revresponse}
	We appreciate the reviewer's comment regarding the boundedness of the Lagrange multipliers in the adaptation law (16a) ((17a) in the present manuscript). 
  ...
  Specifically, the Lyapunov function of output-layer $V_k$ and remaining layers $V_i,\ \forall i\in\{0,\dots,k-1\},$ are defined using tracking errors and adaptive variables including the Lagrange multipliers, as shown below:
  \begin{center}
    \centering
    \includegraphics[width=0.65\linewidth]{figs/r1c2_1.png}
    \includegraphics[width=0.55\linewidth]{figs/r1c2_2.png}
  \end{center}
  where $V_c$ is Lyapunov function regarding the tracking error, and $\sigma_i,\ \forall i\in\{0,\dots,k\},$ are Lagrange multipliers for weight boundedness constraints and $\lambda_j,\ \forall j\in\mathcal{I},$ are Lagrange multipliers for input constraints, with $\mathcal{I}$ being the index set of the input constraints.
...
\begin{revcomment}[2][1]{}
	Equation (35) ((39) in the present manuscript) and the linear independence claim in the paragraph below require verification. The gradient expressions appear inconsistent with standard chain-rule derivation for DNNs. A rigorous derivation must be provided to ensure correctness.
\end{revcomment}
...
\begin{revresponse}
  ...
  In the revised manuscript, we analyzed the linear independence of the gradient of the considered input constraints, and provided the detailed proof in Lines 659--666, as shown below:
  \begin{center}
    \centering
...
\begin{revresponse}
  ...
  In the revised manuscript, we clearly state the rationale for the selection of the comparative methods in Lines 689--700 of Section~V.A, as follows:
  \begin{center}
    \centering
  ...
  It should be noted that the optimization-based methods typically require the knowledge of the system dynamics and relatively high computational burden, which makes them less suitable for application to unknown systems and real-time implementation with limited computational resources, such as the MCU used in our experimental setup.
...
\begin{revresponse}
  ...
  As we mentioned in \textbf{Overview of Major Changes} in the response to editors, the title of the manuscript was revised to clearly reflect the main contribution of the proposed method, which is the capability to handle multiple convex input constraints in the NAC framework.
  ...
  In particular, we have surveyed the advantages and limitations of the existing methods and summarized them in Table~I of the revised manuscript, as shown below:
  % and clearly stated the research gap that the proposed method addresses, 
  \begin{center}
    \centering
    \includegraphics[width=0.99\linewidth]{figs/r1c1_2.png}
  \end{center}
  As shown above, the existing methods have limitations in terms of the types of input constraints they can handle, the computational burden, and the stability guarantees.
...
\begin{revresponse}
  ...
  In Section~V.C (a subsection of validation section) of the revised manuscript, we discussed about the advantages and limitations of the proposed method, as shown below:
  \begin{center}
    \centering
    \includegraphics[width=0.65\linewidth]{figs/r2c2_1.png}
  \end{center}

  We are aware of that even though the proposed method had demonstrated its capability to handle multiple convex input constraints through the real-time implementation, it is only able to handle the constraints in a soft manner, meaning that the constraints may be violated during the adaptation process.
  Therefore, we mentioned that extending our approach to tackle hard constraints to ensure safety, is very important.
  Currently, many methods to incorporate hard constraints are being reviewed, and we are planning to employ them into the proposed method in the future, as stated in Lines 873--880 of the conclusion section.
  \begin{center}
    \centering
...
\begin{revresponse}
  We understand the reviewer's comment as a request for a more detailed clarification of the initialization conditions that facilitate bounded and stable adaptation of the DNN weights.

  As will be discussed in responses to Comments~5 and~8, the exact admissible initial conditions for the control system cannot be explicitly stated because the invariant sets employed in the stability analysis depend on unknown constants.
  ...
  Therefore, in the revised manuscript, we added Remark~2 in the end of Section~IV to provide a guidance on the initialization of the control system, as shown below:
  \begin{center}
    \centering
    \includegraphics[width=0.65\linewidth]{figs/r1c2_3.png}
  \end{center}
  Specifically, we recommend initializing the system state close to the initial desired state to reduce the initial tracking error, assigning small random values to the DNN weights, and setting the Lagrange multipliers to zero by ensuring that there are no initial constraint violations.
  These settings help promote stable online adaptation and reduce the possibility of abrupt control actions caused by the initially untrained DNN, as shown below:

  \bigskip

  Moreover, Remark~3 provides additional guidance for mitigating large initial tracking errors caused by improper system-state initialization and the limited initial performance of the randomly initialized DNN, as shown below:
  \begin{center}
    \centering
  ...
  Together, Remarks~2 and~3 clarify the practical initialization and operating conditions that help satisfy the requirements of the stability analysis and facilitate bounded adaptation of the DNN weights.
...
\begin{revresponse}
  ...
  In addition, the function approximated in the previous submission contained no terms that could become singular and was therefore free of singularities.
  ...
  \begin{center}
    \centering
    \includegraphics[width=0.55\linewidth]{figs/r2c4_1.png}
  \end{center}

  However, the user-defined matrix $\boldsymbol{P}$ is assumed to be bounded and positive definite by its definition, and, the control effectiveness matrix $\boldsymbol{B}$ is guaranteed to be invertible by Assumption~2, as shown below:
  \begin{center}
    \centering
...
\begin{revresponse}
  ...
  Furthermore, we have added Remarks~2--4 in the end of Section~IV to provide practical guidance on initialization, desired trajectory design, and parameter selection, which increases the likelihood that the initial condition requirements of the stability analysis are satisfied.
...
\begin{revresponse}
  ...
  However, depending on the adaptation-law parameters $\alpha$, $\beta_i$, and $\beta_j$, the control signal may exhibit high-frequency oscillations near the constraint boundaries, as discussed in Remark~4 of the revised manuscript.
  ...
  We provided a parametric study on the update rate of power source limit constraint to investigate the effect of the parameter on the control signal.
  ...
  \end{center}
  where (C$_1$) had the higher update rate of the Lagrange multiplier for the power source limit constraint, than (C$_2$), and therefore (C$_1$) exhibited more high-frequency oscillations in the control signal at the extremes of the bounds than (C$_2$).
  Moreover, the Lagrange multipliers of (C$_1$) are repeatedly activated when the constraints are violated and driven back to zero when the constraints are satisfied, resulting in more frequent activation and deactivation than those of (C$_2$). 
  This behavior is consistent with the higher-frequency oscillations observed in the control signal.

  \bigskip

  In conclusion, the constraints are handled by the adaptation law derived from the constrained optimization problem and are satisfied at equilibrium, or approximately under quasi-static conditions. 
  The Lyapunov analysis serves a separate purpose of establishing the stability and boundedness of the closed-loop system and adaptive variables, rather than analyzing the constraint satisfaction directly.
\end{revresponse}
...
\begin{revresponse}
  ...
  \begin{itemize}
    ...
    \item
      However, with lower values of the parameters, the adaptation of the DNN weights and satisfaction of the constraints may be slower and fail to meet the desired performance requirements.
  ...
  In addition, regarding the selection of the architecture of the DNNs, we have added a statement  in Line 745--747 in the revised manuscript, as follows:
  \begin{quotation}
    "
...
\begin{revresponse}
  ...
  \begin{center}
    \includegraphics[width=0.65\linewidth]{figs/r2c8_1.png}
  \end{center}
  Thus, even if the initial tracking error is large, it is expected to decrease while the system follows the warm-up trajectory. 
  Likewise, initializing the DNN weights with small values is an intentional part of the controller design to avoid excessive initial control outputs. 
  These initialization and warm-up procedures are employed because the DNN is not pre-trained; therefore, careful initialization and gradual excitation are important for successful online adaptation of NAC without pre-training.
...
\begin{revresponse}
  We thank the reviewer for this comment. 
  We understand the "the influence of state variable errors on the system's transient response" as referring to the transient tracking performance and their evolution during online adaptation.
  This is because the desired trajectory is time-varying and the transient response of the closed-loop system under the proposed method was sufficiently captured by the results of the real-time implementation.
  ...
  A detailed analysis of these results is provided in Section~V.B of the revised manuscript; briefly, the proposed CONAC implementations (C$_1$) and (C$_2$) achieved larger reductions in the RMS tracking errors than (C$_3$). 
  This difference arises because CONAC directly incorporates all constraints defining the admissible input set into its adaptation process and can therefore utilize the available control authority more effectively. 
  In contrast, the auxiliary-system design of (C$_3$) handles only element-wise input overflow and consequently exhibits more conservative control behavior, as compared in Fig.~10 of the revised manuscript, which is reproduced below.
  \begin{center}
    \includegraphics[width=0.65\linewidth]{figs/r2c10_4.png}
  \end{center}

  This comparison demonstrates that the proposed CONAC method can achieve better transient tracking performance than the existing methods, while still satisfying the input constraints.
\end{revresponse}
...
\begin{revresponse}
  ...
  We understand the comment as a request to clarify how they are handled in the proposed method from both theoretical and practical perspectives and the definition of the convex input constraints.

  \bigskip

  From the theoretical perspective, the previous manuscript did not clearly distinguish how the convex input constraints were incorporated into the constrained optimization problem. 
  ...
  \end{center}
  where $\boldsymbol{r}$ corresponds to the tracking error $\boldsymbol{e}$ in the revised manuscript and $\mathcal{I}$ is the index set of the imposed constraints including both weight and input constraints unlike the revised manuscript which only includes the index set of the input constraints.

  \bigskip

  In the revised manuscript, the weight and input constraints are explicitly distinguished as $c_i^\theta$ and $c_j^u$, respectively, as shown below:
  \begin{center}
    \centering
    \includegraphics[width=0.65\linewidth]{figs/r3c2_1.png}
  \end{center}
  where $\mathcal{K}$ is the index set of the DNN layers which corresponds to the weight constraints  and $\mathcal{I}$ is the index sets of the input constraints only.

  Furthermore, to describe the multiple convex input constraints more clearly, rather than representing the input saturation using a single generic constraint function, the saturation function is now defined using multiple convex constraint functions with respect to the admissible input set $\Omega_u$ according to Assumption~3 in the revised manuscript, as follows:
  \begin{center}
    \centering
    \includegraphics[width=0.65\linewidth]{figs/r3c1_1.png}
  \end{center}

  Then, the Lagrangian function is defined with respect to the constrained optimization problem (14) in the revised manuscript, and the adaptation laws of the DNN weights and Lagrange multipliers are derived from the primal-dual gradient update law, as shown below:
  \begin{center}
    \centering
  ...
  From the practical perspective, we have clarified how the input constraints are handled in the adaptation laws in Lines 387--395 of the revised manuscript, as shown below:
  \begin{center}
    \centering
  ...
  We hope this revised notation makes it explicit how each convex input constraint is incorporated into the adaptation laws to be satisfied, and how the saturation function is defined using multiple convex input constraints.
  
  \bigskip

  Additionally, as a example of the definition of the saturation function, in Section~V of the revised manuscript, the input constraint imposed on the validation is specifically defined as follows:
  \begin{center}
    \centering
    \includegraphics[width=0.65\linewidth]{figs/r3c1_2.png}
  \end{center}
  which can be interpreted as a combination of the upper and lower bounds on the actuator torque, and  the power source limit constraint as follows:
  \begin{center}
    \centering
    \includegraphics[width=0.65\linewidth]{figs/r3c1_4.png}
  \end{center}
  Furthermore, the saturation function is illustrated in Fig.~5(b) in the revised manuscript, as shown below (see, right figure of Fig.~5):
  \begin{center}
    \centering
    \includegraphics[width=0.65\linewidth]{figs/r3c1_3.png}
  \end{center}
...
\begin{revresponse}
  ...
  First, the tracking error $\boldsymbol{e}$ minimization problem under the input constraints $c_j^u,\ \forall j\in\mathcal{I},$ and bounded DNN weights $\widehat{\boldsymbol{\theta}}_i,\ \forall i\in\mathcal{K},$ is formulated as a constrained optimization problem, as shown below:
  \begin{center}
    \centering
    \includegraphics[width=0.65\linewidth]{figs/r3c2_1.png}
  \end{center}
  ...
  \end{center}
  where $\sigma_i,\ \forall i\in\mathcal{K},$ and $\lambda_j,\ \forall j\in\mathcal{I},$ are Lagrange multipliers for the weight boundedness constraints and input constraints, respectively.
  To solve the constrained optimization problem, the primal-dual problem is formulated, as follows:
  \begin{center}
    \centering
  ...
  This adaptation law, then, is incorporated into the control design to ensure that the DNN weights are updated in a manner that minimizes the tracking error while satisfying the input constraints and maintaining boundedness of the weights.
...
\begin{revresponse}
  ...
  The assumptions in the revised manuscript reflect the necessary conditions for the system, and located in the problem formulation section (Section~II), while the preliminaries regarding control design and DNN features are provided in the first section of the control design section (Section~III.A).
...
\begin{revresponse}
  ...
  For instance, as mentioned in the previous response, we have clarified the notation for the convex input constraints by explicitly distinguishing between weight and input constraints as $c_i^\theta,\ \forall i\in\mathcal{K},$ and $c_j^u,\ \forall j\in\mathcal{I},$ respectively, in the revised manuscript.
...
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
\begin{revcomment}[1][1]{}
	The simulation section does not sufficiently highlight the key contributions of the manuscript. Furthermore, the motivations behind some of the simulation results are unclear. For example, what is the purpose of Figure~6 (Figure~9 in the revised manuscript)?
\end{revcomment}
...
\begin{revresponse}
  ...
  Particularly, we zoomed in the experiment results on the time interval of 26.6 s to 27.3 s in Fig.~9 (which corresponds to Fig.~6 in the previous submission) to clearly illustrate the behavior of the control signal and the satisfaction of the constraints during a period when the control signal approaches the boundaries of the convex input constraints.
  \begin{center}
    \centering
    \includegraphics[width=0.65\linewidth]{figs/r3c6_1.png}
  \end{center}
  As shown in Fig.9(b), the control signals of the proposed method, (C$_1$) and (C$_2$), fully utilized the control authority while handling the convex input constraints, whereas the control signal of (C$_3$) was more conservative and did not fully utilize the available control authority, since it only handled element-wise input overflow according to the auxiliary-system design, as compared in Fig.10 of the revised manuscript.
  \begin{center}
    \centering
    \includegraphics[width=0.65\linewidth]{figs/r2c10_4.png}
  \end{center}
  ...
  For the case of (C$_4$) which does not consider input constraints, the control signal failed to satisfy the input constraints with excessively large control outputs and resulted in a large tracking error.

  The remaining figure, Fig.~9(c), shows the parametric study on the update rate of Lagrange multiplier for the power source limit (input norm) constraint to investigate the effect of the update rate on the control signal.
  The study demonstrates the trade-off between constraint-handling speed and control signal smoothness, \ie high update rates lead to faster constraint handling but may result in high-frequency oscillations in the control signal at the boundaries of the constraints, while low update rates lead to slower constraint handling but smoother control signals.
...
\begin{concludingresponse}[]
	Thank you for your valuable comments on our manuscript. 
  Specifically, the clear definition and handling procedures of the convex input constraints, the reorganization of the assumptions and lemmas, and the clarification of the contributions have substantially improved the clarity and logical flow of the manuscript.
\end{concludingresponse}
...
\begin{revresponse}
  ...
  \begin{center}
    ...
    \includegraphics[width=0.65\linewidth]{figs/r4c1_1.png}
  \end{center}
  In addition, we have explicitly stated that the identity-matrix approximation is based on this property in Lines 419--422 on Section~III.C.
\end{revresponse}
...
\begin{revresponse}
  ...
  Thus, in the validations, the architecture of the DNNs was selected empirically by considering both approximation performance and the computational constraints of the target MCU, as stated in Lines 745--747 of the revised manuscript, as follows:
  \begin{quotation}
    "
    Specifically, the architecture of the DNNs are empirically chosen considering the approximation capability and computational efficiency.
    "
...
\begin{revresponse}
  ...
  Regarding system uncertainties, since the proposed method is NAC method which requires no prior knowledge of the system dynamics, it is inherently robust to uncertainties.
  However, we acknowledge that the current validation does not include additional disturbances beyond the uncertainties of the system.
  Therefore, in Lines 873--880 of the conclusion in the revised manuscript, we have added a statement to clarify that the consideration of disturbances is left for future work, as follows:
  \begin{center}
    \centering
    \includegraphics[width=0.65\linewidth]{figs/r2c2_2.png}
  \end{center}
...
\begin{revresponse}
  ...
  We kindly ask for the reviewer’s understanding regarding our decision not to incorporate this suggestion.
...
\begin{revresponse}
  ...
  In Lines 113--124 of the introduction section of the revised manuscript, we have added a discussion of the related works, as follows:
  \begin{center}
    \centering
    \includegraphics[width=0.65\linewidth]{figs/r4c7_1.png}
  \end{center}
  The detailed comparison of the proposed method with the suggested related works is provided in the following paragraphs.
  ...
  Unlike \cite{Zhang:2024ab} and \cite{Qin:2025aa}, the optimal control law is derived from the Hamilton-Jacobi-Bellman (HJB) equation, instead of the HJI equation, while the optimal cost function is approximated by a CNN similarly to \cite{Zhang:2024ab} and \cite{Qin:2025aa}.
  ...
  In contrast, the proposed CONAC can handle a broader class of convex input constraints, including element-wise bounds, norm constraints, and other coupled convex input constraints, by explicitly incorporating them into the constrained optimization problem and deriving the adaptation law from the primal-dual formulation.
  ...
  Moreover, utilizing game theory to handle actuator failures or disturbances, as in \cite{Zhang:2024ab, Qin:2025aa}, may also be a potential extension of the proposed method.
  Thus, we left regarding statements in Lines 873--880 of conclusion section in the revised manuscript, as follows:
  \begin{center}
    \centering
    \includegraphics[width=0.65\linewidth]{figs/r2c2_2.png}
  \end{center}
